...
int main(int argc, char* argv[]) {
  ...
  std::string path = "/usr/local/packages/opencv_app/label";
  for (const auto & file : std::filesystem::directory_iterator(path))
       std::cout << file.path() << std::endl;

    std::vector<std::string> class_list;
    std::string path_ = "/usr/local/packages/opencv_app/label/labels.txt";
    ...
    std::cout << class_list[5]<<"helllllllooooooo"<<std::endl;
  ...
  // Mat fg;
 // Mat image12; //to recongnize the Aruco
  ...
  while (true) {
    ...
    // detect aruco ids from frames
    //nv12_mat.copyTo(image12);
    cv::aruco::detectMarkers(nv12_mat,dictionary,corners,ids);
    //std::string res;
    if (ids.size() > 0){
      for (const int &i:ids)
